...
guilds= sourceSession.query(config.guild).all()
for guild in guilds:
    userid1 = str(guild).replace(",","")
    userid2 = userid1.replace(")", "")
    userid3 = userid2.replace("(", "")
    print(userid3)
    exists = sourceSession.query(config).filter_by(guild=userid3).first()
    tr = config(exists.guild, exists.lobby, exists.agelog, exists.modlobby, exists.general)
    session.add(tr)
    session.commit()
