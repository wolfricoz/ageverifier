...
wuser= sourceSession.query(warnings.uid).all()
for uid in wuser:
    userid1 = str(uid).replace(",","")
    userid2 = userid1.replace(")", "")
    userid3 = userid2.replace("(", "")
    print(userid3)
    exists = sourceSession.query(warnings).filter_by(uid=userid3).first()
    intuid = int(exists.uid)
    tr = warnings(intuid, exists.swarnings)
    session.add(tr)
    session.commit()
