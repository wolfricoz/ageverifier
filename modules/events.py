                ...
                if message.channel.id == c.lobby:
                    if match:
                        ...
                        if int(match.group(1)) < 18:
                            await channel.send(
                                f"<@&{p.lobbystaff}> {message.author.mention} has given an age under the age of 18: {message.content} (Note: These are not logged)")
                        elif int(match.group(1)) >= 18:
                            await channel.send(
                                f"<@&{p.lobbystaff}> {message.author.mention} has given {message.content}. You can let them through with `/approve`")
                        return
                    else:
                        try:
